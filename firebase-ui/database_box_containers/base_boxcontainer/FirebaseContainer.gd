...
func _ready():
    if Firebase.Auth.auth.keys().empty():
...
func on_login_succeeded(_auth_token):
    connect_to_database()
...
func on_new_update(data):
    if data.data:
        ...
        on_item_added(item, null, template)
       
func on_item_added(item, key, template):
#    yield(get_tree(), "idle_frame")
#    get_parent().scroll_vertical = get_parent().get_v_scrollbar().max_value
    pass
